...
def find_Obstacles(angle_step):
    ...
    for i in range(-90,90):
        ...
        x = 49 + np.int(np.sin(np.radians(i*angle_step))*tmp/10.5)
        y = np.int(np.cos(np.radians(i*angle_step))*tmp/10.5)
        print("X:", x)
        print("Y:", y)
        if abs(x)>99:
            x=99
...
#            a = detect4ver2.main()
#             print(a)
            if detect4ver2.main() == 0:
#                 time.sleep(5)
                 print('Stopping car, detected stop sign')
                 time.sleep(5)     
#                 continue
            print(detect4ver2.main())
            move_picar(move)
