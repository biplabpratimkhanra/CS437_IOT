...
for _ in range(350):
    ...
    bounding_boxes = sign_detec_SM.detected(frame)
    for road_sign, bb in bounding_boxes.items():
        if len(bb) > 0:
            if save_movie:
                for (x, y, w, h) in bb:
                    cv2.rectangle(frame, (x, y), (x + w, y + h), road_sign.value, 2)
                    cv2.putText(frame, road_sign.name, (x, y), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1.0, color=road_sign.value)
            else:
                print(road_sign.name)
   
    if save_movie:
        fps_num = fps()
        cv2.putText(frame, f"fps: {fps_num:.2f}", (10, 28), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1.0, color=(128, 255, 128))
        out.write(frame)
    else:
        print(fps())
